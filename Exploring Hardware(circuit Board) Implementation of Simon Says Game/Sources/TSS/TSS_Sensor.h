...
***********************************************************************************************************************/
  ...
  /* undefinition of TSI0 (CMSIS header). Defined back at the end of this file */
  #ifdef TSI0_BASE
    #undef TSI0
  #endif
   ...
   *******************************************************/
    ...
    #elif (defined(MCU_MK20D10) || defined(MCU_MK20D5) || defined(MCU_MK20D7) || defined(MCU_MK20DZ10) || defined(MCU_MK20F12))
      #define TSS_MCU_SUBFAMILY_K20        1
    #elif (defined(MCU_MK15E7))
      #define TSS_MCU_SUBFAMILY_K15        1
    #elif (defined(MCU_MKL02Z4) || defined(MCU_MKL04Z4) || defined(MCU_MKL05Z4) || defined(MCU_MKL14Z4) || defined(MCU_MKL15Z4) || defined(MCU_MKL24Z4) || defined(MCU_MKL25Z4) || defined(MCU_MKL34Z4))
      #define TSS_MCU_SUBFAMILY_KLX5       1
    #elif (defined(MCU_MKL16Z4) || defined(MCU_MKL26Z4) || defined(MCU_MKL36Z4) || defined(MCU_MKL46Z4))
      #define TSS_MCU_SUBFAMILY_KLX6       1
    #endif
  #endif
   ...
   *******************************************************/
      ...
      #ifndef PORT_PCR_PS_MASK
        #define TSS_GPIO_VERSION           3
      #else
        #define TSS_GPIO_VERSION           1
      #endif
    #elif TSS_CFM_MCU
   ...
   ******************** Noise Mode ***********************
   *******************************************************/
  #ifndef TSS_TSI_SUPPORTS_NOISE_MODE
    #if (defined(TSS_MCU_SUBFAMILY_KLX5) || defined(TSS_MCU_SUBFAMILY_KLX6))
      #define TSS_TSI_SUPPORTS_NOISE_MODE       1
    #else
      #define TSS_TSI_SUPPORTS_NOISE_MODE       0
    #endif
  #endif
   ...
   *******************************************************/
    ...
    typedef uint16_t (* TSS_SAMPLE_ELECRODE_LOW) (void);
    typedef uint8_t (* TSS_METHOD_CONTROL) (uint8_t u8ElNum, uint8_t u8Command);
  ...
  #else
    typedef struct {
      const uint8_t gen_cu8ModuleID;
      const int8_t gen_ci8AmplitudeFilterDeltaLimit;
    } TSS_GENERIC_ELECTRODE_ROMDATA;

    typedef struct {
      uint16_t gen_u16AmplitudeFilterBase;
  ...
  extern uint16_t tss_u16CapSample;
  extern volatile uint8_t tss_u8HWTimerFlag;
  extern volatile uint8_t tss_u8SampleIntFlag;  /* Extern Variable Flag for sample interrupted */

  /* Electrode Constants & Variables */
  extern const uint8_t * const tss_acp8ElectrodeROMData[];
  extern const uint8_t * tss_acp8ElectrodeRAMData[];

  #if TSS_HCS08_MCU || TSS_CFV1_MCU
    #if TSS_USE_SIMPLE_LOW_LEVEL
      #if (TSS_GPIO_VERSION == 2)
        extern volatile uint8_t *tss_pu8ElectrodePortAdr;
        extern volatile uint8_t *tss_pu8ElectrodePortControlAdr;
        extern volatile uint8_t tss_u8ElectrodePortMask;
      #else
        extern volatile uint8_t *tss_pu8ElectrodePortAdr;
        extern volatile uint8_t tss_u8ElectrodePortMask;
      #endif
      ...
      #if (TSS_GPIO_VERSION == 2)
        extern volatile uint8_t *tss_psElectrodeTimertAdr;
        extern volatile uint8_t *tss_pu8ElectrodePortAdr;
        extern volatile uint8_t *tss_pu8ElectrodePortControlAdr;
        extern volatile uint8_t tss_u8ElectrodePortMask;
      #else
  ...
  #elif TSS_KINETIS_MCU
    extern volatile uint32_t *tss_psElectrodeTimertAdr;
    extern volatile uint32_t *tss_psElectrodeRGPIOAdr;
    extern volatile uint8_t tss_u8ElectrodeBitNum;
  #elif TSS_CFM_MCU
    extern volatile uint32_t *tss_psElectrodeTimertAdr;
    extern volatile uint32_t *tss_psElectrodePortAdr;
    extern volatile uint32_t *tss_psElectrodePCTLAdr;
    extern volatile uint8_t tss_u8ElectrodeBitNum;
  #endif

  #ifdef TSS_USE_NOISE_AMPLITUDE_FILTER
    #if TSS_USE_NOISE_AMPLITUDE_FILTER
      extern uint8_t TSS_u16NoiseAmplitudeFilter(uint16_t u16CapSubSample, uint16_t *u16AmplitudeFilterBase, int8_t i8AmplitudeFilterDeltaLimitTemp);
      extern uint8_t TSS_i8NoiseAmplitudeFilter(int8_t i8SCapSubSample, int8_t *i8AmplitudeFilterBase, int8_t i8AmplitudeFilterDeltaLimitTemp);
    #endif
  #endif
  ...
  brief:      Initializes the HW Timer, KBI, PTI and Timer
              Input Capture peripherals for the sensing of the electrodes.
  param:      uint8_t u8Command
  return:     Status code
  */

  extern uint8_t (* const tss_faSampleElectrode[])(uint8_t u8ElecNum, uint8_t u8Command);
  /*
  function:   tss_faSampleElectrode
  brief:      Sampling Control
  param:      uint8_t u8ElecNum - Electrode Number
              uint8_t u8Command - Command for Measurement routine
  return:     uint16_t Measured capacitance value
  */

  extern uint16_t (* const tss_faSampleElectrodeLow[])(void);
  /*
  ...
  */
  ...
  brief:      Performs a valid reading of one electrode, stores the timer
              values and returns a status code
  ...
  */
  ...
  /*
  function:   TSS_SensorInitCallback
  ...
  */
  ...
  extern void TSS_LowPowerCalibrationEnable(void);
  /*
  function:   TSS_LowPowerCalibrationEnable
  brief:      Function enables calibration of TSI threshold on 2N22D silicon.
              ...
              The function needs to be called after or before TSS_LOWPOWER_EN
  ...
  */
  ...
  /****************************************************************************
  *  If there is not defined Electrode Type then set to electrode to default
  *  GPIO type, or if is not used electrode set UNUSED.
  ****************************************************************************/
  ...
  ****************************************************************************/
  ...
  /* Revert define back to it's value */
  #ifdef TSI0_BASE
